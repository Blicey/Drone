...
{
	float termI=0;
	//deltaT应该是us为单位，所以除了1e6，估计是从systick中得到的。
...
void CtrlAttiAng(void)
{
		...
		if(altCtrlMode==MANUAL){
			//RC就是Remote Control的缩写，是一个结构体，包含Roll,pitch,yaw,THROTTLE
			//得到指令之后，设定参数到angTarget中
			angTarget[ROLL]=(float)(RC_DATA.ROOL);
			angTarget[PITCH]=(float)(RC_DATA.PITCH);
		}else{
			//如果是自动控制，那么设定值为rollSp，估计是PID的输出
